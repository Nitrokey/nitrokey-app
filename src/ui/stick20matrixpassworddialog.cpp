...
int height, width;
    ...
    ui->pushButton_0->
        setGeometry (QRect
                     (Rect.x () + width * i / 10 + width / 20 - 10, 355, 20,
                      20));
    i++;
    ui->pushButton_1->
        setGeometry (QRect
    ...
    ret =
        connect (WaitForMatrixTimer, SIGNAL (timeout ()), this,
                 SLOT (CheckGetPasswordMatrix ()));

    WaitForMatrixTimer->start (100);

    Stick20MatrixDataReceived = 0;
    HID_Stick20MatrixPasswordData_st.StatusFlag_u8 =
        STICK20_PASSWORD_MATRIX_STATUS_IDLE;
...
QString OutputText;
    ...
    if (false == cryptostick->isConnected)
    {
        csApplet->
            warningBox
            ("MatrixPasswordDialog: No cryptostick 2.0 connected!");
        return;
    }
    ...
    if (TRUE == SetupMode)
    {
        ...
        for (i = 0; i < 10; i++)
        {
            for (i1 = 0; i1 < 10; i1++)
            {
                HID_Stick20MatrixPasswordData_st.PasswordMatrix_u8[i1 * 10 +
                                                                   i] = i;
            }
        }
    }
    else
    {
        ...
        ui->label->
            setText ("Waiting for matrix data generated by cryptostick 2.0");
    }
    if (ret)
    {
    }   // Fix warnings
...
void MatrixPasswordDialog::sendPasswordKey (char* Password)
{
    ...
    if (InputPasswordLengthPointer < OUTPUT_CMD_STICK20_MAX_MATRIX_ROWS)
    {
        OutputText =
            "Select column " +
            QString ("%1").arg (InputPasswordLengthPointer + 1);
        ui->label->setText (OutputText);
    }
    else
    {
...
int text_height;
    ...
    if (FALSE == SetupMode)
    {
        if (0 == Stick20MatrixDataReceived)
        {
            ...
            painter.drawText (20, height / 2,
                              "Waiting for matrix data generated by cryptostick 2.0");
            return;
        }
    }
    ...
    for (i = 0; i < 10; i++)
    {
        color.setRgb (100 + 50 * (i % 2), 100 + 50 * (i % 2),
                      100 + 50 * (i % 2));
    ...
    if ((false == HideMatrix) && (false == SetupMode))
    {
        ...
        if (-1 != HighLightVerticalBar)
        {
            for (i = 0; i < 10; i++)
            {
                color.setRgb (50 + 50 * (i % 2), 50 + 50 * (i % 2),
                              50 + 50 * (i % 2));
                painter.fillRect (width * HighLightVerticalBar / 10,
                                  height * i / 10, width / 10, height / 10,
                                  color);
            }
        }
    }
    ...
    if (true == SetupMode)
    {
        ...
        {
            color.setRgb (200, 200, 200);
            painter.fillRect (0, height / 10 * SetupModeActiveRow, width,
                              height / 10, color);
        }
    }
...
QString text;
            ...
            text =
                '0' +
                HID_Stick20MatrixPasswordData_st.PasswordMatrix_u8[i * 10 +
                                                                   i1];
            painter.drawText (width * i / 10 + (width / 20) - 6,
                              (height * (i1 + 1) / 10) - text_height + 2,
                              text);
        }
    }
}
...
void MatrixPasswordDialog::RowSelected (int SelectedRow)
{
    ...
    {
        OutputText =
            "Select row " + QString ("%1").arg (SelectedRowCounter + 1);
        ui->label->setText (OutputText);
    }
    else
    {
...
QMouseEvent* e;
                ...
                e = (QMouseEvent *) event;
                SetupModeActiveRow =
                    e->pos ().y () * 10 / ui->frame->size ().height ();
                ui->frame->repaint ();
                return true;
            }
    ...
    if (target == ui->pushButton_Send)
    {
        if (event->type () == QEvent::MouseButtonRelease)
        {
            if (false == SetupMode)
            {
                ...
                ret =
                    cryptostick->
                    stick20SendPasswordMatrixPinData (SelectedColumns);
                if (TRUE == ret)
                {
                }
            }
