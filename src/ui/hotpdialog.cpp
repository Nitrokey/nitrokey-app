...
QDialog (parent), ui (new Ui::HOTPDialog)
{
    ...
    connect (TOTP_ValidTimer, SIGNAL (timeout ()), this,
             SLOT (checkTOTP_Valid ()));
...
QString output;
    ...
    device->getCode (slotNumber, lastTOTPTime / lastInterval, lastTOTPTime,
                     lastInterval, result);
...
void HOTPDialog::setToTOTP ()
{
    ...
    ui->intervalSpinBox->setValue (device->TOTPSlots[slotNumber - 0x20]->
                                   interval);
...
QPalette palette;
    ...
    checkTime =
        (lastTOTPTime / (uint64_t) lastInterval) * (uint64_t) lastInterval;
    ...
    if ((checkTime + (uint64_t) lastInterval - (uint64_t) 5 <= currentTime)
        && (checkTime + lastInterval > currentTime))
    {
...
uint64_t currentTime;
    ...
    if (currentTime >= lastClipboardTime + (uint64_t) 60
        && (QString::compare (clipboard->text (), ui->lineEdit->text ()) ==
            0))
    {
